...
import { getTodos, getCheckboxes, countCompletedTodos } from "../src/playwright/todos-locators"
import { deleteTodos, addTodos, checkTodos, uncheckAllTodos } from "../src/playwright/todos-helpers";
...
test.describe("todos features", () => {
  ...
  test("status", async ({ todosPage }) => {
    ...
    for (let i = 0; i <= total; i++) {
      ...
      await expect.poll(() => todosPage.locator("#list-heading").textContent())
  ...
  test.skip("more actions", async ({ todosPage }) => {
